    ...
    S: A2dpMode,
{
    ...
    /// Unregister PCM callback before disconnect/stop (Bluedroid flush can pass invalid buf).
    pub fn clear_source_data_callback(&self) -> Result<(), EspError>
    where
        S: SourceEnabled,
    {
        esp!(unsafe { esp_a2d_source_register_data_callback(None) })
    }

    pub fn restore_source_data_callback(&self) -> Result<(), EspError>
    where
        S: SourceEnabled,
    {
        esp!(unsafe {
    ...
    unsafe extern "C" fn source_data_handler(buf: *mut u8, len: i32) -> i32 {
        ...
        // Bluedroid A2DP source read size; larger values are invalid during teardown.
        const MAX_SOURCE_READ: usize = 8192;
        if len > MAX_SOURCE_READ {
            return 0;
        }
        let event = A2dpEvent::SourceData(core::slice::from_raw_parts_mut(buf, len));
        debug!("Got event {{ {:#?} }}", event);
